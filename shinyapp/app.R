...
car_meta_path <- file.path("data", "CAR_T", "GSE297676_meta.txt")
car_expr_rds_path <- file.path("data", "CAR_T", "GSE297676_logNormalized_data.rds")  

if (!file.exists(car_meta_path))
  stop("Missing CAR-T meta file: ", car_meta_path, "\nPlace it under data/CAR_T/")
if (!file.exists(car_expr_rds_path))
  stop("Missing CAR-T expression RDS: ", car_expr_rds_path, "\nPlace the .rds under data/CAR_T/")
...
car_explicit_pre_file <- file.path("data","CAR_T","DE_per_study_pre","GSE297676","DE_GSE297676_rawIDs_withSymbols.tsv")
...
ui <- navbarPage(
  ...
  tabPanel(
    ...
    fluidPage(
      br(),
      h4("Coming soon"),
      p("Planned: select target gene; compare KO vs WT across cell types.")
    )
  )
)
...
server <- function(input, output, session) {
  ...
  observeEvent(input$top_tabs, {
    # ----- PAN-T Cell-level -----
    if (identical(input$top_tabs, "Cell-level") && is.null(gene_index_rv())) {
